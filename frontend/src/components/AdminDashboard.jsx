...
const AdminDashboard = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/admin/create-college', formData);
  ...
  const handleDeleteConfirm = async () => {
    ...
    setDeleting(true);
    try {
      await axios.delete(`http://localhost:5000/api/admin/delete-college/${collegeToDelete._id}`);
